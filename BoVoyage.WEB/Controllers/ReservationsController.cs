...
namespace BoVoyage.WEB.Controllers
{
    public class ReservationsController : BaseController
    {
        ...
        public ActionResult Create(int customer, int travel, ReservationViewModel reservationViewModel, int[] insurancesID)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        var nb = bookingfile.Travelers == null ? 1 : bookingfile.Travelers.Count;
                        return RedirectToAction("AddTraveler", new { bookingfile = bookingfile.ID, traveler = nb });
                    }
                }
                else
                {
        ...
        [Authentication(Type = "CUSTOMER")]
        public ActionResult AddTraveler(TravelerViewModel model)
        {
            if (model == null)
                return HttpNotFound();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    var bookingFile = this.serviceBookingFile.GetBookingFileWithTravelersAndInsurancesIncluded(model.BookingFileID);
                    ...
                    var traveler = new Traveler()
                    {
                        ...
                        SequentialNb = string.Format("{0}-{1}", bookingFile.SequentialNb, bookingFile.Travelers.Count + 1)
                    };
